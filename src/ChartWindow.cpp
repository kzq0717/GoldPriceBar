...
void ChartWindow::setupChart()
{
    ...
    m_currentSeries->setMarkerSize(14);
    ...
    m_tipLabel->setStyleSheet(
        "QLabel{background-color:rgba(33,37,41,230);color:#fff;border:1px solid #666;"
    ...
    auto mkValue = [](const QString& s, const QString& color) {
        auto* l = new QLabel(s);
        l->setStyleSheet(QStringLiteral("color:%1;font-size:16px;font-weight:bold;").arg(color));
        l->setWordWrap(true);
        return l;
    };
...
void ChartWindow::onNewPrice(double price, double, const QString&)
{
    ...
    updateSidePanelValues(price > 0 ? price : (m_plotPoints.isEmpty() ? 0.0 : m_plotPoints.last().second),
                          m_lastPredictPrice, m_hasPredict, high, low, m_forecastModeTag);

    // 预测：至少间隔 30 秒才重算，避免侧栏预测值持续跳动
    const qint64 nowMs = QDateTime::currentMSecsSinceEpoch();
    if (m_lastForecastMs == 0 || (nowMs - m_lastForecastMs) >= kForecastIntervalMs) {
        updateForecast();
    }
}
...
int ChartWindow::nearestPointIndex(qreal xMsecs) const
{
    if (m_plotPoints.isEmpty())
        return -1;
    int best = 0;
    qreal bestDist = qAbs(static_cast<qreal>(m_plotPoints.first().first.toMSecsSinceEpoch()) - xMsecs);
    for (int i = 1; i < m_plotPoints.size(); ++i) {
        const qreal d = qAbs(static_cast<qreal>(m_plotPoints.at(i).first.toMSecsSinceEpoch()) - xMsecs);
        if (d < bestDist) {
            bestDist = d;
            best = i;
        }
 ...
 * 仅供参考，不构成投资建议
 */
QVector<QPair<QDateTime, double>> ChartWindow::computeForecastLocal(int horizonSec) const
{
...
void ChartWindow::applyForecastPoints(const QVector<QPair<QDateTime, double>>& forecast, const QString& modeTag)
{
    ...
    if (m_axisX) {
        ...
        const QDateTime needMax = ((now > forecast.last().first) ? now : forecast.last().first).addSecs(3600);
...
void ChartWindow::updateSidePanelValues(double current, double predict, bool hasPredict,
                                       double high, double low, const QString& modeTag)
{
    ...
    if (m_sideModeLabel)
        m_sideModeLabel->setText(tr("模式: %1").arg(modeTag.isEmpty() ? tr("本地") : modeTag));
}
...
void ChartWindow::requestOnlineForecast()
{
    ...
    const QString systemPrompt =
        QStringLiteral("你是黄金短线分析助手。根据用户提供的最近分时点，推断未来约120秒可能的价格路径。"
                       "只输出一个 JSON 对象，不要 Markdown，不要解释。格式："
                       "{\"points\":[{\"offset_sec\":10,\"price\":0.0}],\"brief\":\"一句话理由\"}。"
    ...
    request.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("GoldPriceBarLite/0.2.0"));
...
void ChartWindow::onOnlineForecastFinished(QNetworkReply* reply)
{
    ...
    QString content = choices.at(0).toObject()
    ...
    const QJsonDocument predDoc = QJsonDocument::fromJson(content.toUtf8(), &perr);
...
void ChartWindow::updateHighLowMarkers()
{
    ...
    m_highSeries->append(static_cast<qreal>(hp.first.toMSecsSinceEpoch()), hp.second);
    m_lowSeries->append(static_cast<qreal>(lp.first.toMSecsSinceEpoch()), lp.second);

    // 数值统一刷到右侧信息栏
    const double cur = m_plotPoints.last().second;
    updateSidePanelValues(cur, m_lastPredictPrice, m_hasPredict, hp.second, lp.second, m_forecastModeTag);
}
...
void ChartWindow::fetchChartFromApi()
{
    ...
    const QUrl url(QStringLiteral("https://jin.20021002.xyz/api.php?action=chart&type=%1")
                       .arg(currentTypeCode()));
    QNetworkRequest request(url);
    request.setHeader(QNetworkRequest::UserAgentHeader, QStringLiteral("GoldPriceBarLite/0.1.6"));
...
void ChartWindow::updateSeries()
{
    ...
    if (m_plotPoints.isEmpty()) {
        ...
        const QDateTime now = QDateTime::currentDateTime();
        m_axisX->setRange(QDateTime(QDate::currentDate(), QTime(0, 0)), now.addSecs(3600));
    ...
    const QString typeName = currentTypeCode() == QStringLiteral("gj")
                                 ? tr("伦敦金")
                                 : (currentTypeCode() == QStringLiteral("ms")
                                        ? tr("民生")
                                        : tr("浙商"));
...
void ChartWindow::resizeEvent(QResizeEvent* event)
{
    ...
    if (!m_plotPoints.isEmpty()) {
        ...
        HistoryCache::instance().todayHigh(high);
        HistoryCache::instance().todayLow(low);
        updateSidePanelValues(m_plotPoints.last().second, m_lastPredictPrice, m_hasPredict,
                              high, low, m_forecastModeTag);
    }
}
